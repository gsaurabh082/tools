...
Write-Host ""
Write-Host "RDP Host Manager is starting at $url" -ForegroundColor Green
...
try {
    & $pythonCommand.Source @pythonPrefix -m uvicorn rdp_manager_app:app --host 127.0.0.1 --port $Port
} finally {
    Pop-Location
    if ($openJob) {
