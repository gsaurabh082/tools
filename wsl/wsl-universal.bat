...
REM ============================================================
REM wsl-universal.bat
REM
REM A parameterized version of wsl-local.bat (same folder) - no
REM hardcoded username, OneDrive tenant, or project path anywhere
REM in this file. Everything machine/project-specific is passed
REM in as a command-line argument, or - if you just double-click
REM it - it asks for it interactively.
REM
REM Usage (all optional, unset ones are asked for interactively):
REM   wsl-universal.bat -Folder "C:\path\to\wsl" ^
REM       -Distro Ubuntu -Port 2375 -TestUtils "C:\path\to\TestUtils"
REM
REM Run "wsl-universal.bat -?" to see all options.
REM
REM What it does:
...
REM   6. Sync Windows to that daemon (docker context + this window's DOCKER_HOST)
...
REM ------------------------------------------------------------
REM 6/7 - Sync Windows to the WSL docker daemon
REM ------------------------------------------------------------
echo.
echo [6/7] Syncing Windows to the WSL docker daemon...
REM NOTE: wsl-network-fix.ps1's Pre phase (step 3 above) deliberately clears
REM any *persisted* DOCKER_HOST (User/Machine scope) every run, because a
REM stale persisted value previously caused Testcontainers to silently use
REM the wrong IP forever - .testcontainers.properties (written by
REM wsl-setup.sh) is the intended single source of truth for that. So this
REM script does NOT re-persist DOCKER_HOST (setx) - that would just
REM reintroduce the exact bug already fixed. Instead:
REM   - a docker *context* is created/updated (persists in
REM     %USERPROFILE%\.docker\contexts, untouched by the clearing above) so
REM     NEW terminals running plain "docker" pick up the WSL daemon; and
REM   - DOCKER_HOST is set for THIS window only, same as wsl-local.bat.
...
echo   ^(persists across new terminals; Docker Desktop, if installed, can reset
echo   this on its own startup - re-run this script if that happens^)

set "DOCKER_HOST=tcp://localhost:%DOCKER_TCP_PORT%"
echo   Set DOCKER_HOST for THIS window only ^(matches wsl-local.bat and avoids
echo   the stale-persisted-DOCKER_HOST bug wsl-network-fix.ps1 guards against^).
...
:ShowUsage
echo Usage: wsl-universal.bat [options]
